...
void init_id(void)
{
	fstream fs(DATA_ID_PATH,ios::in|ios::binary);
	if(fs.good()) return;
	
	int id[3] = {10,100,1000};
	fs.open(DATA_ID_PATH,ios::out|ios::binary);
	fs.write((const char*)id,sizeof(id));
	fs.close();
}
...
int get_mgrid(void)
{
	fstream fs(DATA_ID_PATH,ios::binary|ios::in|ios::out);
	if(!fs)
	{
